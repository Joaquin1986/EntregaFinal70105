...
const buildResponse = (data, type, site, sort, category) => {
    ...
    if (type === 'views') {
        ...
        firstLink = `${baseURL}views/${site}?limit=${data.limit}&page=1`;
        lastLink = `${baseURL}views/${site}?limit=${data.limit}&page=${data.totalPages}`
        if (sort) {
