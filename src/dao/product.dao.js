...
class ProductDao {
    ...
    // Crea un array de productos en la BD
    static async addManyProducts(products) {
        try {
            const result = await productModel.create(products);
            if (result) {
                console.log(`✅ ${products.length} productos agregados exitosamente a la BD`);
                return result;
            }
            return false;
        } catch (error) {
            throw new Error(`⛔ Error: No se pudieron agregar ${products.length} productos a la BD => error: ${error.message}`);
        }
    }
